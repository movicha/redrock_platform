...
public class CSVReader extends InputStreamReader {
	...
	protected boolean parseBoolean(String column) {
		if (column == null)
			return false;
		else if (column.equalsIgnoreCase("y") || column.equalsIgnoreCase("yes")
				|| column.equalsIgnoreCase("true")
				|| column.equalsIgnoreCase("1"))
			return true;
		else
			return false;
	}

	protected String getColumn(String[] titles, String[] values, String column)
			throws Exception {
		int c = -1;
		String s = "";
		...
		if (c == -1)
			return ""; // throw new
						// Exception("unable to find column "+column+" in "+s.substring(1));
