...
public class CSVProcessor {
	...
	public class DataReader extends CSVReader {
		...
		private void processLine(String[] titles, String[] values)
				throws Exception {
			count++;
			String src = loop;
			while (src.contains("[%")) {
	...
	private void buildTemplate(String template) throws Exception {
		...
		if (i < 0)
			throw new Exception(
					"Unable to process template - didn't find [%loop");
		start = template.substring(0, i);
		template = template.substring(i + 6);
		i = template.indexOf("%]");
		if (i < 0)
			throw new Exception(
					"Unable to process template - didn't find %] matching [%loop");
		String tmp = template.substring(0, i);
		if (tmp != null && !tmp.equals("")) {
			if (!tmp.startsWith(" count="))
				throw new Exception(
						"Unable to process template - unrecognised content on [%loop");
		...
		if (i < 0)
			throw new Exception(
					"Unable to process template - didn't find [%endloop%]");
