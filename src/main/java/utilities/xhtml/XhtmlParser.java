...
public class XhtmlParser {
	...
	public XhtmlNode parseHtmlNode(XmlPullParser xpp) throws Exception {
		...
		for (int i = 0; i < xpp.getAttributeCount(); i++) {
			res.getAttributes().put(xpp.getAttributeName(i),
		...
		while (eventType != XmlPullParser.END_TAG) {
			if (eventType == XmlPullParser.TEXT) {
				...
				throw new Exception("Unhandled XHTML feature: "
						+ Integer.toString(eventType) + descLoc());
	...
	private String descLoc() {
		return " at line " + Integer.toString(line) + " column "
	...
	public XhtmlDocument parse(String source, String entryName)
			throws Exception {
		rdr = new StringReader(source);
		return parse(entryName);
	}

	public XhtmlDocument parse(InputStream input, String entryName)
			throws Exception {
	...
	private XhtmlDocument parse(String entryName) throws Exception {
		...
		if (!n.equals(entryName))
			throw new Exception(
					"Unable to Parse HTML - does not start with 'html'"
							+ descLoc());
	...
	private void parseElementInner(XhtmlNode node, List<XhtmlNode> parents)
			throws Exception {
		StringBuilder s = new StringBuilder();
		while (peekChar() != '\0' && !parents.contains(unwindPoint)
				&& !(node == unwindPoint)) {
			if (peekChar() == '<') {
				addTextNode(node, s);
				readChar();
				if (peekChar() == '!')
					...
					if (node.getName().equals(n))
						...
						if (mustBeWellFormed)
							throw new Exception("Malformed XHTML: Found \"</"
									+ n + ">\" expecting \"</" + node.getName()
									+ ">\"" + descLoc());
						...
						if (unwindPoint != null) {
							for (int i = parents.size(); i > 0; i--) {
								if (i < parents.size()
										&& parents.get(i) == unwindPoint)
									return;
								if (i == parents.size()) {
									parents.get(i - 1).getChildNodes()
											.addAll(node.getChildNodes());
									node.getChildNodes().clear();
								} else {
									parents.get(i - 1)
											.getChildNodes()
											.addAll(parents.get(i)
													.getChildNodes());
					...
					throw new Exception("Unable to Parse HTML - node '"
							+ node.getName() + "' has unexpected content '"
							+ peekChar() + "' (last text = '" + lastText + "'"
							+ descLoc());
			} else if (peekChar() == '&') {
				parseLiteral(s);
			} else
	...
	private void parseElement(XhtmlNode parent, List<XhtmlNode> parents)
			throws Exception {
	...
	private void parseAttributes(XhtmlNode node) throws Exception {
		...
		while (peekChar() != '>' && peekChar() != '/' && peekChar() != '\0') {
			String name = readName();
			if (name.length() == 0) {
				throw new Exception("Unable to read attribute on <"
						+ node.getName() + ">" + descLoc());
			}
			while (Character.isWhitespace(peekChar()))
				readChar();

			if (isNameChar(peekChar()) || peekChar() == '>'
					|| peekChar() == '/')
				node.getAttributes().put(name, null);
			else if (peekChar() != '=') {
				throw new Exception("Unable to read attribute value on <"
						+ node.getName() + ">" + descLoc());
			} else {
				readChar();
				while (Character.isWhitespace(peekChar()))
				...
				if (peekChar() == '"' || peekChar() == '\'')
					node.getAttributes().put(name,
							parseAttributeValue(readChar()));
				else
			...
			if (peekChar() == '!') {
				...
				if (peekChar() == '-') {
					...
					if (peekChar() == '-') {
						...
						throw new Exception("unrecognised element type <!"
								+ peekChar() + descLoc());
				} else
	...
	private String readToTagEnd() throws Exception {
		...
		if (peekChar() != '\0') {
			...
			throw new Exception("Unexpected termination of html source"
					+ descLoc());
		return s.toString();
	}
	...
	private void parseLiteral(StringBuilder s) throws Exception {
		...
		if (c.equals("apos"))
			...
			s.append((char) 402); // latin small f with hook = function =
			...
			s.append((char) 933); // greek capital letter upsilon, U+03A5
									// ISOgrk3
			...
			s.append((char) 962); // greek small letter final sigma, U+03C2
									// ISOgrk3
		else if (c.equals("sigma"))
			...
			s.append((char) 8230); // horizontal ellipsis = three dot leader,
									// U+2026 ISOpub
		else if (c.equals("prime"))
			s.append((char) 8242); // prime = minutes = feet, U+2032 ISOtech
		else if (c.equals("Prime"))
			s.append((char) 8243); // double prime = seconds = inches, U+2033
									// ISOtech
			...
			s.append((char) 8465); // blackletter capital I = imaginary part,
									// U+2111 ISOamso
		else if (c.equals("real"))
			s.append((char) 8476); // blackletter capital R = real part symbol,
									// U+211C ISOamso
		else if (c.equals("trade"))
			s.append((char) 8482); // trade mark sign, U+2122 ISOnum
		else if (c.equals("alefsym"))
			s.append((char) 8501); // alef symbol = first transfinite cardinal,
									// U+2135 NEW
			...
			s.append((char) 8629); // downwards arrow with corner leftwards =
			...
			s.append((char) 8709); // empty set = null set = diameter, U+2205
									// ISOamso
		else if (c.equals("nabla"))
			s.append((char) 8711); // nabla = backward difference, U+2207
									// ISOtech
			...
			s.append((char) 8719); // n-ary product = product sign, U+220F
									// ISOamsb
			...
			s.append((char) 8764); // tilde operator = varies with = similar t
									// U+223C ISOtech
		else if (c.equals("cong"))
			s.append((char) 8773); // approximately equal to, U+2245 ISOtec
		else if (c.equals("asymp"))
			s.append((char) 8776); // almost equal to = asymptotic to, U+2248
									// ISOamsr
		else if (c.equals("ne"))
			...
			s.append((char) 8855); // circled times = vector product, U+2297
									// ISOamsb -->
		else if (c.equals("perp"))
			s.append((char) 8869); // up tack = orthogonal to = perpendicular,
									// U+22A5 ISOtech
			...
			s.append((char) 9001); // left-pointing angle bracket = bra, U+2329
									// ISOtech
		else if (c.equals("rang"))
			s.append((char) 9002); // right-pointing angle bracket = ket, U+232A
									// ISOtech
			...
			s.append((char) 9829); // black heart suit = valentine, U+2665
									// ISOpub
		else if (c.equals("diams"))
			s.append((char) 9830); // black diamond suit, U+2666 ISOpub --
		else
			throw new Exception("unable to parse character reference '" + c
					+ "'' (last text = '" + lastText + "'" + descLoc());
	}
